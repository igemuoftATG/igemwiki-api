...
const { debug, getUsernameAndPassword } = require('./util.js')
...
const login = ({ username, password, loginEndpoint = LOGIN_ENDPOINT } = {}) => {
  // If BOTH username and password are not provided, use environment variables
  if (username === undefined && password === undefined) {
    username = process.env.username
    password = process.env.password
  }
  // If either username or password is still undefined, use a prompt
  if (username === undefined || password === undefined) {
    const promptResults = getUsernameAndPassword()
    username = promptResults.username
    password = promptResults.password
  }
