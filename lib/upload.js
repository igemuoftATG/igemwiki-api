...
const postMultiform = ({
  ...
  return rp({
    ...
    debug(`Gonna ensure ${path.dirname(outfile)} exists`)
    return Promise.all([
      Promise.resolve(body),
      fsp.ensureDir(path.dirname(outfile))
    ])
  }).then(([ body ]) => {
...
const upload = ({
  ...
  const baseUrl = type === TYPE_IMAGE ? _baseUrl : _baseUrl + '/' + pageOrImageName
  ...
  return Promise.all(preChecks).then((resolvedValues) => {
    ...
    console.log(`${responseDetails.status} ${responseDetails.source} -> ${responseDetails.target}`)
    return(responseDetails)
  })
}
